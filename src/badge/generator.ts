...
interface BadgeDimensions {
  ...
  badgeRadius: number
  charWidth: number
  countCharWidth: number
  countPadX: number
  fontSize: number
  labelSidePad: number
  logoCircleSize: number
}
...
export function generateModernBadge(count: number): string {
  ...
  const countTextWidth = Math.max(formattedCount.length * DIMENSIONS.countCharWidth, 12)

  const labelSectionWidth = leftStripPad + labelTextWidth + DIMENSIONS.labelSidePad
